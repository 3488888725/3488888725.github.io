...
function tonav() {
  ...
  document.getElementById("page-name").innerText = document.title.split(" | 林阿斗的百宝袋")[0];
}
...
$.ajax({
  ...
  data: {
    key: 'LCGBZ-DH2CN-N6QFA-SX4LI-PLD6O-KRBT7',  // 这里要写你的KEY!!!
    output: 'jsonp',
  },
  dataType: 'jsonp',
...
function showWelcome() {
  ...
  let dist = getDistance(117.17403, 31.75406, ipLoacation.result.location.lng, ipLoacation.result.location.lat); //这里换成自己的经纬度
  ...
  try {
    ...
    document.getElementById("welcome-info").innerHTML =
      `<b><center>🎉 欢迎信息 🎉</center>&emsp;&emsp;欢迎来自 <span style="color:var(--blue-custom)">${pos}</span> 的小伙伴，${timeChange}您现在距离站长约 <span style="color:var(--theme-color)">${dist}</span> 公里，当前的IP地址为： <span style="color:var(--blue-custom)">${ip}</span>， ${posdesc}</b>`;
  } catch (err) {
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by 林阿斗的百宝袋 %c 你正在访问林阿斗的百宝袋",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| 林阿斗的百宝袋") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('林阿斗的百宝袋的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
function createtime() {
  ...
  var start = new Date("26/04/2023 00:00:00"); // 旅行者1号开始计算的时间
  var dis = Math.trunc(23400000000 + ((now - start) / 1000) * 17); // 距离=秒数*速度 记住转换毫秒
  var unit = (dis / 149600000).toFixed(6);  // 天文单位
  // 网站诞生时间
  var grt = new Date("26/04/2023 00:00:00");
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    --default-bg: url(https://s2.loli.net/2023/04/23/X6SFc4ultPIyJ5V.jpg);
    --darkmode-bg:url(https://s2.loli.net/2023/04/29/UCcGFIXyrOvE23W.jpg);
    --mobileday-bg: url(https://s2.loli.net/2023/04/29/2fWQPgdChaKeUSr.jpg);
    --mobilenight-bg: url(https://s2.loli.net/2023/04/29/QIrtnCz6kb1uAOj.jpg);
  }`;
}
// 切换背景主函数
function changeBg(s) {
